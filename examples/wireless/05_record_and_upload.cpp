...
int main(int argc, char** argv) {
    ...
    // rejected, so re-runs need the earlier one deleted (`ef record delete`).
    ...
    status = device.upload_recording(name, url);
    if (status != ERROR_CODE::SUCCESS) {
        std::cerr << "upload_recording failed: " << to_string(status) << "\n";
        device.close();
        return 1;
    }
