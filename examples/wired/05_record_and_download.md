---
title: "Record and download (wired)"
...
duplicate name is rejected, so a re-run needs `ef record delete <name>` first):
...
status = device.enable_recording(rp);
```
...
status = device.download_recording(name, dest);
```

## Run it

```sh
./build/wired_05_record_and_download        # record 3 s, then download
./build/wired_05_record_and_download 10     # record 10 s
```

## Expected output

Progress lines for recording and downloading, ending with
`example_recording.mcap` saved in the working directory.
